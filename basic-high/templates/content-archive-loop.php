			...
			while ($cat_query->have_posts()):
				...
				<article <?php post_class('post-content'); ?>>
					<h2 class="post-title"><a href="<?php the_permalink() ?>" rel="bookmark"
							title="<?php the_title_attribute(); ?>"><?php echo mb_strimwidth(get_the_title(), 0, 32, '...'); ?></a>
					</h2>
					...
					<footer class="entry-meta">
						...
						<span class="author vcard"><?php echo get_avatar(get_the_author_meta('ID'), 32); ?> <a
								...
								title="<?php printf(__(' View all articles by %s ', 'basic-high'), the_author_meta('display_name', 25)); ?>">
						...
						if ($categories_list):
							...
							<span class="cat-links">
								<?php printf(__('<span class="%1$s">Posted in</span> %2$s ', 'basic-high'), 'entry-utility-prep entry-utility-prep-cat-links', $categories_list);
								$show_sep = true; ?>
							</span>
